...
namespace HalkUIEditor {
    ...
    public sealed class LosslessXml {
        ...
        public LosslessXml(string source) {
            ...
            foreach(Match m in Tokens.Matches(source)) {
                ...
                // Structural keys are assigned once after parsing.
    ...
    public sealed class UiDocument {
        ...
        public bool Atlas { get {if(atlasCache.HasValue)return atlasCache.Value;bool value=Xml.Root.Name=="ui_texture";if(!value)foreach(var n in Xml.Nodes)if(n.Name=="texture"&&n.Has("id")&&AtlasReference(n)!=""){value=true;break;}atlasCache=value;return value;} }
        ...
        public void SetText(string text){var parsed=new LosslessXml(text);Text=text;Xml=parsed;ParseError=null;atlasCache=null;}
        public void Commit(string before){if(before==Text)return;UndoStack.Push(before);RedoStack.Clear();TrimHistory();}
        void TrimHistory(){if(UndoStack.Count<2)return;var all=UndoStack.ToArray();long bytes=0;int keep=0;while(keep<all.Length&&keep<200){long next=(long)all[keep].Length*2;if(keep>0&&bytes+next>32L*1024*1024)break;bytes+=next;keep++;}if(keep==all.Length)return;UndoStack.Clear();for(int i=keep-1;i>=0;i--)UndoStack.Push(all[i]);}
        public void RestoreText(string text){Text=text;atlasCache=null;try{Xml=new LosslessXml(text);ParseError=null;}catch(FormatException ex){Xml=new LosslessXml("");ParseError=ex.Message;}}
    ...
    public sealed class Workspace : IDisposable {
        ...
        public List<string> IndexWarnings=new List<string>();
        Dictionary<string,string> resolvedPaths=new Dictionary<string,string>(StringComparer.OrdinalIgnoreCase),imageFailures=new Dictionary<string,string>(StringComparer.OrdinalIgnoreCase);
        ...
        public TextureRegion ResolveState(string id,string state){if(string.IsNullOrEmpty(id))return null;TextureRegion r;string baseId=Regex.Replace(id,@"_[ehtd]$","");if(!string.IsNullOrEmpty(state)&&Textures.TryGetValue(baseId+"_"+state,out r))return r;return Resolve(id);}
        ...
        public Bitmap Image(string path){if(path==null)return null;string failure;if(imageFailures.TryGetValue(path,out failure))throw new InvalidDataException(failure);Bitmap b;if(images.TryGetValue(path,out b)){imageOrder.Remove(path);imageOrder.AddLast(path);return b;}try{b=TextureLoader.Load(path);}catch(Exception ex){imageFailures[path]=ex.Message;throw;}long size=(long)b.Width*b.Height*4;while(imageBytes+size>160L*1024*1024&&imageOrder.Count>0){string old=imageOrder.First.Value;imageOrder.RemoveFirst();var im=images[old];imageBytes-=(long)im.Width*im.Height*4;im.Dispose();images.Remove(old);}images[path]=b;imageOrder.AddLast(path);imageBytes+=size;return b;}
        public void ClearImageCache(){foreach(var b in images.Values)b.Dispose();images.Clear();imageOrder.Clear();imageFailures.Clear();resolvedPaths.Clear();imageBytes=0;}
        public void ReloadTextures(){ClearImageCache();var original=new List<string>(Files);Files.Clear();ImageFiles.Clear();Scan(Root,0);foreach(string p in original)if(!Files.Contains(p)&&Documents.ContainsKey(p)&&Documents[p].Dirty)Files.Add(p);Files.Sort(StringComparer.OrdinalIgnoreCase);ImageFiles.Sort(StringComparer.OrdinalIgnoreCase);Reindex();}
        public void Dispose(){ClearImageCache();}
    ...
    public static class UiLayout {
        public static UiNode TextureNode(UiNode n,string state){return n.Child("texture_"+state)??n.Child("texture")??n.Child("texture_e");}
        public static string TextureName(UiDocument d,UiNode n,string state){if(d.Atlas)return d.AtlasReference(n);var t=TextureNode(n,state);return t==null?"":t.Content(d.Text);}
        ...
        public static List<string> Validate(UiDocument d,Workspace w){var warnings=new List<string>(d.Xml.Diagnostics);if(d.ParseError!=null)warnings.Add(d.ParseError);warnings.AddRange(w.IndexWarnings);var missing=new HashSet<string>();foreach(var n in d.Xml.Nodes){if(n.Geometry){foreach(string a in new[]{"x","y","width","height","cell_width","cell_height"})if(n.Has(a)){float v;if(!float.TryParse(n.Get(a,""),NumberStyles.Float,CultureInfo.InvariantCulture,out v)||float.IsNaN(v)||float.IsInfinity(v))warnings.Add(n.Name+": некорректный "+a);else if(a!="x"&&a!="y"&&v<0)warnings.Add(n.Name+": отрицательный "+a);}
