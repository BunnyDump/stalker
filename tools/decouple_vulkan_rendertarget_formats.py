...
// D3D9 fallback conversion is intentionally centralized here.
enum XrRtFormat
{
...
def decouple(root: Path) -> None:
    ...
    print(f"[vulkan-rendertarget-formats] centralized {applied} legacy format uses")


def main() -> int:
    ap = argparse.ArgumentParser(description="Centralize render-target formats behind logical format names.")
